...
function processSum(num,cb) { 
  ...
  return cb(num.reduce(reducer, 0))};
...
function getFullNames(runners) {
  let fullNames = []
  runners.forEach(cur =>  fullNames.push(`${cur.last_name}, ${cur.first_name}`));
  return fullNames
}
...
function firstNamesAllCaps(runners) {
  const upperFirst = runners.map(cur => cur.first_name.toUpperCase())
  return upperFirst
}
...
function getRunnersByTShirtSize(runners, tShirtSize) {
  const shirtSize = runners.filter(cur => {return cur.shirt_size === tShirtSize});
...
function tallyUpDonations(runners) {
  return runners.reduce((acc, cur) => {return acc + cur.donation;}, 0);
